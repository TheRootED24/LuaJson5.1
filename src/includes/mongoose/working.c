...
void update_rlen(json_elm *elm, elm_rlen *erl) {
    ...
    else if (erl->unsub) { // Handles both EXT and NIL cleanup
...
void lua_json_elm_get_val_length2(lua_State *L, json_elm *elm, elm_rlen *erl) {
    ...
    switch(vtype) {
        case LUA_TUSERDATA:
        {
            ...
            if(!erl->is_exval) erl->sub = nested;
            else               erl->unsub = nested;
...
json_elm* lua_json_elm_index(lua_State *L, bool new) {
	json_elm *elm = new ? check_json_elm(L, -3) : check_json_elm(L, -2);
	int8_t pos = new ? 4 : 3; // new = new_index
	int8_t idx = pos - 2;
    // Handle "Environment Index" edge case
    if(lua_istable(L, -pos) && lua_isuserdata(L, -1)) {
        lua_replace(L, -pos);
        if(lua_isnumber(L, -idx)) {
            elm->idx = luaL_checkinteger(L, -idx);
			elm->check_idx(elm);
		}
        ...
        if(lua_isnumber(L, -idx)) {
            elm->idx = (luaL_checkinteger(L, -idx) + 1);
			elm->check_idx(elm);
		}
    }
...
lua_json_elm_stringify(lua_State *L) {
	...
	if(!DEBUG) {
		...
		printf("marhsaled len: %ld\nrender len: %ld\n", lt, __lua_json_elm_get_rlen(elm, elm->mode, elm->escape));
		printf("\n########### %ld/%ld ###########\n\n", lt, __lua_json_elm_get_rlen(elm, elm->mode, elm->escape));
...
lua_json_elm_len(lua_State *L) {
    json_elm *elm = check_json_elm(L, 1);
    if(elm) {
        lua_pushinteger(L, elm->base->rlen-1);
	}
...
int lua_json_elm_gc(lua_State *L) {
	json_elm *elm = check_json_elm(L, 1);
	if(DEBUG) printf("Destroy Elm: %ld\n", elm->id);
	//if(elm->dom_id)
			//free(elm->dom_id);
