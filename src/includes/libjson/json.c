...
static int lua_pass_string(const char *s, int len) {
  int i;
  for (i = 0; i < len; i++) {
    if (s[i] == '\\' && i + 1 < len && json_esc(s[i + 1], 1)) {
      i++;
    } else if (s[i] == '\0') {
      return LUA_JSON_INVALID;
    } else if (s[i] == '"') {
      return i;
    }
  }
...
static double lua_atod(const char *p, int len, int *numlen) {
  double d = 0.0;
  int i = 0, sign = 1;
...
size_t _lua_json_next(struct lua_str obj, size_t ofs, struct lua_str *key,
  ...
  if (ofs >= obj.len) {
    ...
    struct lua_str sub = lua_str_n(obj.buf + ofs, obj.len - ofs);
    if (ofs == 0) ofs++, sub.buf++, sub.len--;
    if (*obj.buf == '[') {  // Iterate over an array
      int n = 0, o = _lua_json_get(sub, "$", &n);
      if (n < 0 || o < 0 || (size_t) (o + n) > sub.len) {
        ofs = 0;  // Error parsing key, stop scanning
      } else {
        if (key) *key = lua_str_n(NULL, 0);
        if (val) *val = lua_str_n(sub.buf + o, (size_t) n);
    ...
    // LUA_INFO(("SUB ofs %u %.*s", ofs, sub.len, sub.buf));
...
int _lua_json_get(struct lua_str json, const char *path, int *toklen) {
  ...
  unsigned char nesting[LUA_JSON_MAX_DEPTH];
  ...
  if (toklen) *toklen = 0;
  if (path[0] != '$') return LUA_JSON_INVALID;

#define LUA_CHECKRET(x)                                  \
  do {                                                  \
...
#define LUA_EOO(x)                                            \
  do {                                                       \
    if (depth == ed && ci != ei) return LUA_JSON_NOT_FOUND;   \
    if (c != nesting[depth - 1] + 2) return LUA_JSON_INVALID; \
    depth--;                                                 \
    LUA_CHECKRET(x);                                          \
  } while (0)
  ...
  for (i = 0; i < len; i++) {
    ...
    switch (expecting) {
      case S_VALUE:
        ...
        if (depth == ed) j = i;
        if (c == '{') {
          if (depth >= (int) sizeof(nesting)) return LUA_JSON_TOO_DEEP;
          ...
          int numlen = 0;
          lua_atod(&s[i], len - i, &numlen);
          i += numlen - 1;
        } else if (c == '"') {
          int n = lua_pass_string(&s[i + 1], len - i - 1);
          if (n < 0) return n;
          i += n + 1;
        } else {
          return LUA_JSON_INVALID;
      ...
      case S_KEY:
        if (c == '"') {
          int n = lua_pass_string(&s[i + 1], len - i - 1);
          if (n < 0) return n;
          if (i + 1 + n >= len) return LUA_JSON_NOT_FOUND;
          if (depth < ed) return LUA_JSON_NOT_FOUND;
          if (depth == ed && path[pos - 1] != '.') return LUA_JSON_NOT_FOUND;
...
struct lua_str _lua_json_get_tok(struct lua_str json, const char *path) {
  int len = 0, ofs = _lua_json_get(json, path, &len);
  return lua_str_n(ofs < 0 ? NULL : json.buf + ofs,
                  (size_t) (len < 0 ? 0 : len));
}

bool _lua_json_get_num(struct lua_str json, const char *path, double *v) {
  int n, toklen, found = 0;
  if ((n = _lua_json_get(json, path, &toklen)) >= 0 &&
      (json.buf[n] == '-' || (json.buf[n] >= '0' && json.buf[n] <= '9'))) {
    if (v != NULL) *v = lua_atod(json.buf + n, toklen, NULL);
    found = 1;
  }
  return found;
}
...
bool _lua_json_get_bool(struct lua_str json, const char *path, bool *v) {
  int found = 0, off = _lua_json_get(json, path, NULL);
...
bool _lua_json_unescape(struct lua_str s, char *to, size_t n) {
  size_t i, j;
  for (i = 0, j = 0; i < s.len && j < n; i++, j++) {
    if (s.buf[i] == '\\' && i + 5 < s.len && s.buf[i + 1] == 'u') {
      ...
      if (lua_str_to_num(lua_str_n(s.buf + i + 2, 4), 16, &to[j],
...
char *_lua_json_get_str(struct lua_str json, const char *path) {
  char *result = NULL;
  int len = 0, off = _lua_json_get(json, path, &len);
  if (off >= 0 && len > 1 && json.buf[off] == '"') {
    if ((result = (char *) calloc(1, (size_t) len)) != NULL &&
        !_lua_json_unescape(lua_str_n(json.buf + off + 1, (size_t) (len - 2)),
                          result, (size_t) len)) {
      free(result);
      result = NULL;
    }
...
char *_lua_json_get_b64(struct lua_str json, const char *path, int *slen) {
  char *result = NULL;
  int len = 0, off = _lua_json_get(json, path, &len);
  if (off >= 0 && json.buf[off] == '"' && len > 1 &&
      (result = (char *) calloc(1, (size_t) len)) != NULL) {
    size_t k = _lua_base64_decode(json.buf + off + 1, (size_t) (len - 2), result,
                                (size_t) len);
...
char *_lua_json_get_hex(struct lua_str json, const char *path, int *slen) {
  char *result = NULL;
  int len = 0, off = _lua_json_get(json, path, &len);
  if (off >= 0 && json.buf[off] == '"' && len > 1 &&
    ...
    int i;
    for (i = 0; i < len - 2; i += 2) {
      lua_str_to_num(lua_str_n(json.buf + off + 1 + i, 2), 16, &result[i >> 1],
                    sizeof(uint8_t));
    }
    result[len / 2 - 1] = '\0';
...
long _lua_json_get_long(struct lua_str json, const char *path, long dflt) {
  double dv;
  long result = dflt;
  if (_lua_json_get_num(json, path, &dv)) result = (long) dv;
  return result;
}
